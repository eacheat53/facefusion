...
# ========== 工具函数 ==========
def _pick_valid_template(name: str | None) -> str:
    """
    把传入模板名规范化（空格->下划线），并在 WARP_TEMPLATE_SET 中选一个可用键。
    优先：规范化同名 -> 任意包含 'styleganex' 的 -> 任意包含 'ffhq' 的 -> 任意可用键。
    """
...
def _norm_size(val, default_edge: int) -> tuple[int, int]:
    """
    尺寸统一为 (w,h)。标量->方形；None->默认边长；元组/列表->取前两位并转 int。
    """
    if val is None:
        return (default_edge, default_edge)
...
>>>>>>> f3be23d19be842c1ddda7f6a94f6efe0f1157538
    ...
    with thread_semaphore():
        out = age_modifier.run(None, age_modifier_inputs)[0][0]
...
def modify_age(target_face: Face, temp_vision_frame: VisionFrame) -> VisionFrame:
    ...
    extend_vision_frame, extend_affine_matrix = warp_face_by_face_landmark_5(
        temp_vision_frame, extend_face_landmark_5, tmpl_bg, size_bg_wh
    )
